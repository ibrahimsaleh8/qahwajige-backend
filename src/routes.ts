...
import { adminArticlesRouter } from "./articles/ai-articles.routes";
...
router.use("/api/admin/articles", adminArticlesRouter);
router.use("/api/v1/seo", seoRouter);
const upload = multer();
...
router.put("/api/article/:articleId", async (req: Request, res: Response) => {
  try {
    ...
    const body = req.body || {};
    const { title, content, coverImage, categorySlug, keywords } = body;
